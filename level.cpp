        ...
        for (int i = 0; i < levelSize; i++) {
            for (int j = 0; j < levelSize; j++) {
                ...
                if (this->bytes[k][i * levelSize + j] == '|'){
                    this->objects.push_back(Object({(float) x ,(float) y,z},z, {1.0f,1.0f,1.0f}, false,"cube", this->models["stone_brick_dark"]));
                }
                if (this->bytes[k][i * levelSize + j] == '1'){
                    this->objects.push_back(Object({(float) x ,(float) y,z},z, {1.0f,1.0f,1.0f}, true, "cube", this->models["stone_brick_dark"]));
                }
                if (this->bytes[k][i * levelSize + j] == '2'){
                    this->objects.push_back(Object({(float) x ,(float) y,z},z, {1.0f,1.0f,1.0f}, true, "cube", this->models["stone_brick_light"]));
                }

                if (this->bytes[k][i * levelSize + j] == 'f'){
                    this->objects.push_back(Object({(float) x ,(float) y,z},z, {1.0f,1.0f,1.0f}, true, "floor", this->models["stone_brick_light"]));
                }
                ...
                if (this->bytes[k][i * levelSize + j] == char(53425)) {
                    cout << string(1,this->bytes[k][i*levelSize + j]);
                    vector<Texture2D> texs;
